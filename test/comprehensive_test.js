...
// 测试1: 检查必要的文件和目录是否存在
runTest('Check required directories and files', () => {
...
// 测试2: 测试新闻抓取器
runTest('Test News Scraper functionality', () => {
  const NewsScraper = require(path.join(scrapersDir, 'news_scraper'));
  const scraper = new NewsScraper();
  
  // 测试比特币新闻抓取
  const bitcoinNews = scraper.scrapeNews('bitcoin', 20);
  if (!bitcoinNews || !Array.isArray(bitcoinNews.articles) || bitcoinNews.articles.length < 10) {
    throw new Error(`Bitcoin news insufficient: got ${bitcoinNews?.articles?.length || 0} articles, expected >=10`);
  }
  ...
  // 测试以太坊新闻抓取
  const ethereumNews = scraper.scrapeNews('ethereum', 20);
  if (!ethereumNews || !Array.isArray(ethereumNews.articles) || ethereumNews.articles.length < 10) {
    throw new Error(`Ethereum news insufficient: got ${ethereumNews?.articles?.length || 0} articles, expected >=10`);
  }
...
// 测试3: 测试技术分析生成器
runTest('Test Technical Analysis Generator', () => {
...
// 测试4: 测试数据聚合流程
runTest('Test Data Aggregation Pipeline', () => {
  // 运行数据聚合脚本
  try {
    const output = execSync('node scrapers/data_aggregator.js', { 
    ...
    if (btcNews.articles.length < 50 || ethNews.articles.length < 50) {
      throw new Error(`Insufficient news articles: BTC=${btcNews.articles.length}, ETH=${ethNews.articles.length}, expected >=50 each`);
    }
...
runTest('Test API Endpoints', () => {
  ...
  // 模拟API调用
  const apiHandler = require(path.join(__dirname, '../pages/api/news/v2/[coin]'));
...
// 测试6: 验证真实URL链接
runTest('Validate Real News URLs', () => {
  const btcNewsPath = path.join(testDataDir, 'bitcoin_news_v2.json');
